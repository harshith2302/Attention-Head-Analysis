#!/usr/bin/env python3
"""
Targeted Fix Script: Bypasses the lm-eval 'dataclass' bug by explicitly 
initializing the TaskManager, and retries any remaining failed tasks.
"""
...
def run_single_task(lm, task_cfg, batch_size):
    ...
    # THE FIX: Explicitly create the TaskManager to bypass the bug
    tm = TaskManager()
    
    try:
        return lm_eval.simple_evaluate(
            model=lm, tasks=[task_cfg["lm_eval"]], num_fewshot=task_cfg["fewshot"],
            batch_size=batch_size, task_manager=tm, confirm_run_unsafe_code=True
        )
    except TypeError:
        return lm_eval.simple_evaluate(
            model=lm, tasks=[task_cfg["lm_eval"]], num_fewshot=task_cfg["fewshot"],
            batch_size=batch_size, task_manager=tm
        )
...
def process_failed_json(json_path, lm, masker, global_csv, task_dir, batch_size):
    ...
    print(f"\n{'='*70}")
    print(f"🔄 FIXING DATACLASS BUG FOR: {cfg['tag']}")
...
def main():
    ...
    print("Loading model for fast targeted fix...")
